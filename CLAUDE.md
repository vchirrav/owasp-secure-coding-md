...
AI agent skills (SAST, SCA, DAST, container scanning, secret detection, etc.) live in a separate repository: [product-security-ai-skills](https://github.com/vchirrav/product-security-ai-skills). The `secure-coding-audit` and `secure-coding-generate` skills in that repo depend on the `rules/` folder from this repository.
